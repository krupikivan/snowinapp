    ...
    await facebookSignIn.logIn(['email']).then((fbResponse) {
      ...
      switch (fbResponse.status) {
        case FacebookLoginStatus.loggedIn:
          http
              .get(
                  'https://graph.facebook.com/v2.12/me?fields=name,first_name,last_name,email,picture.height(200)&access_token=${fbResponse.accessToken.token}')
              .then((graphResponse) {
            var profile = json.decode(graphResponse.body);
            print(profile.toString());

            LoginProvider()
                .fbLogin(fbResponse.accessToken.token)
                .then((response) {
              if (response['success']) {
